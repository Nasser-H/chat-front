...
export default function Login() {
    ...
    async function login(values) {
        let {data} = await axios.post('http://127.0.0.1:8000/api/login',values);
        if (data.token){
    ...
    return <>
        <section className="py-10 px-12">
            ...
            <form onSubmit={formik.handleSubmit} className="max-w-md mx-auto pt-10">
                 <div className="text-white text-lg  mb-6 flex justify-center hidden">
                    <div className="bg-red-500 w-fit py-1 px-3 flex">
                        <div className="me-1 mt-[6.9px] rounded-full size-4 border bg-white flex justify-center items-center"><i className="fa-solid fa-exclamation text-sm text-main"></i></div>Failed : </div>
                </div>
                ...
                <p className="text-sm text-gray-600 mt-4">
                    ...
                    <Link to={"/register"} className="text-main hover:underline ps-1">
                        Create one here
                    </Link>
                </p>
            </form>
